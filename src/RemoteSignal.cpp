...
void RemoteSignal::update()
{
  refreshTime();

  // client side ping no needed.  server do.
  ...
  uint8_t *message = (uint8_t *)cong._buffer;
    ...
    if(  message[0] == Boho::MsgType::ENC_488 ){
      ...
      /*
      !! End to End Encryption Mesasge decryption. 
      ** payload is enc_pack.
    ...
    switch(hd){
      ...
      case RemoteSignal::MsgType::SIGNAL:{
          ...
          if( directMessageTopic ){
            if(messageCallback) messageCallback( directMessageTopic, payloadType, message + 3 + tagLen , payloadLen   );
          }else{
            if(messageCallback) messageCallback( tagStr, payloadType, message + 3 + tagLen , payloadLen   );
          }
        break;
      }
...
bool RemoteSignal::overflow()
{
  if( cong._state > 100 ) return true;
    else return false; 
}
...
void RemoteSignal::login( char* auth_id, char* auth_key  )
{
...
void RemoteSignal::auth( char* auth_id, char* auth_key  )
{
...
void RemoteSignal::set( char* setString ){
    int setLen = strlen(setString);  
    if( setLen > 255 ) return; 
...
void RemoteSignal::subscribe( char* tag ){
...
void RemoteSignal::signal2( char* target ,char* topic,  const char* data  ){
...
void RemoteSignal::signal2( char* target ,char* topic,  const char* data1, const char* data2   ){
  int targetLen = strlen(target);
  int topicLen = strlen(topic);
  char tag[ targetLen + topicLen + 1 ];
  memcpy( tag, target , targetLen );
...
// signal : no payload
void RemoteSignal::signal( char* tag ){
    int tagLen = strlen(tag);  
    if( tagLen > 255 ) return;
...
void RemoteSignal::signal( char* tag, const char* data  ){
    int tagLen = strlen(tag); 
    if( tagLen > 255 ) return;
...
void RemoteSignal::signal( char* tag, const char* data1 , const char* data2  ){
    int tagLen = strlen(tag); 
    if( tagLen > 255 ) return;
...
void RemoteSignal::signal( char* tag, const uint8_t* data , uint32_t dataLen ){
...
void RemoteSignal::signal2( char* target ,char* topic,  const uint8_t* data , uint32_t dataLen ){
  int targetLen = strlen(target);
  int topicLen = strlen(topic);
  char tag[ targetLen + topicLen + 1 ];
  memcpy( tag, target , targetLen );
...
void RemoteSignal::signal_e2e( char* tag, const uint8_t* data , uint32_t dataLen , const char *dataKey){
  ...
  if(  encMode == RemoteSignal::ENC_MODE::YES || 
    encMode == RemoteSignal::ENC_MODE::AUTO && isAuthorized ){
    // 암호채널,  시그널 헤더를 암호화한다.  enc_pack은 암호화하지 않는다.
  ...
  //buf size: enc_e2e header(21) + RemoteSignalHeader( 3+tagLen) |  encpack(25) + dataLen 
    ...
    // encrypt sigHeader only. exclude payload 
...
void RemoteSignal::signal2_e2e( char* target ,char* topic,  const uint8_t* data , uint32_t dataLen , const char *dataKey){
  int targetLen = strlen(target);
  int topicLen = strlen(topic);
  if( (targetLen + topicLen) > 255 ) return;
